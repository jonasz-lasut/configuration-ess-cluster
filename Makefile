# Project Setup
PROJECT_NAME := configuration-ess-cluster
...
uptest: $(UPTEST) $(KUBECTL) $(KUTTL)
	...
	@KUBECTL=$(KUBECTL) KUTTL=$(KUTTL) CROSSPLANE_NAMESPACE=$(CROSSPLANE_NAMESPACE) $(UPTEST) e2e examples/ess-xr.yaml --setup-script=test/setup.sh --default-timeout=2400 || $(FAIL)
	@$(OK) running automated tests

e2e: build controlplane.up local.xpkg.deploy.configuration.$(PROJECT_NAME) uptest

render:
	crossplane beta render examples/ess-xr.yaml apis/composition.yaml examples/functions.yaml -r
